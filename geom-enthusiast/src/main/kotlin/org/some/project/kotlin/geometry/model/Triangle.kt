...
data class Triangle(
    ...
    fun show(short: Boolean = false, withTags: Boolean = false): String {
        ...
        if (!short) {
            val side1Info = side1.show(short = false, withTags = withTags)
            val side2Info = side2.show(short = false, withTags = withTags)
            val side3Info = side3.show(short = false, withTags = withTags)
